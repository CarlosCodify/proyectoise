  ...
  module V1
    class UsuariosController < ApplicationController
      ...
      def index
        ...
        render json: @usuarios.as_json(include: [{ contacto: { only: %i[id nombre apellido_materno
                                                                        apellido_paterno] } }])
      end

      # GET /api/v1/usuarios/1
      def show
        render json: @usuario.as_json(include: [{ contacto: { only: %i[id nombre apellido_materno
                                                                       apellido_paterno] } }])
      end
